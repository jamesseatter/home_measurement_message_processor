...
public class MeasurementDAO {
    ...
    private String dbMeasurementTable;
    ...
    public int insertRecord(final Measurement sr) {
        ...
        if (validateResult == null) {
            ...
            final String sql = "INSERT INTO " + dbMeasurementTable + " (record_id,date_measured_utc,sensor_type,sensor_id,title,description,measurement_unit,value,low_threshold,high_threshold,alert_group,alert_destination, alert_uid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private String validateRecord(Measurement record) {
        if ((record.getMeasureTimeUTC() == null) || (record.getMeasureTimeUTC().toString().equals(""))) {return "MeasureTimeUTC";}
        if ((record.getAlertID() == null) || (record.getAlertID().toString().equals(""))) { record.setAlertID(UUID.fromString( "00000000-0000-0000-0000-000000000000" ));}
        if ((record.getTitle() == null) || (record.getTitle().equals(""))) {record.setTitle("");}
        if ((record.getDescription() == null) || (record.getDescription().equals(""))) {record.setDescription("");}
        return null;
    }
}
